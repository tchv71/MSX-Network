...
public:
	...
	void addByteEsc(unsigned char ch)
	{
	...
	void addWordEsc(unsigned int w)
	{
		addByteEsc(w >> 8);
		addByteEsc(w & 255);
	}
	...
	bool SendPacket(int cmdType, unsigned char* buf, int len, int dstAddr = 0, int srcAddr = 0)
	{
		...
		case NET_ERASE_FILE:
			msg = msg ? msg : "NET_ERASE_FILE";
		case NET_WRITE_FILE:
			msg = msg ? msg : "NET_WRITE_FILE";
			if (verbose) printf("Send %s:\n", msg);
			setHeader(Header, sizeof(Header));
			addByteEsc(cmdType);		// COMMAND
			addWordEsc(len);			// LENGTH: FCB (37 bytes) + H, F, A (3 bytes)
			addBufEnd(buf, len);	// PAYLOAD + CHECHKSUM + TERMINATOR
			//{
			//	int crcShift = 5+2+4;
			//	checkCrc(txbuf.c_str()+crcShift, txbuf.size()-5-crcShift);
			//}
			return write_wait(msg);
		case NET_MASTER_DATA:
			...
			{
				setHeader(Header, sizeof(Header));
				addByteEsc((l == len) ? NET_MASTER_DATA : NET_MASTER_DATA2);

				addWordEsc((l > MAXBLKSIZE) ? MAXBLKSIZE : l);
	...
	bool SendFile(FILE* infile, const char* name, int size)
	{
		...
		SendPacket(NET_ERASE_FILE, (unsigned char*)&TxData, sizeof(TxData));
		...
		for (int sectNo = 0; !feof(infile); sectNo++)
		{
			...
			if (!SendPacket(NET_WRITE_FILE, (unsigned char*)&RxData, sizeof(RxData)))
				return false;
		...
		if (!SendPacket(NET_CLOSE_FILE, (unsigned char*)&RxData, sizeof(RxData)))
			return false;
		printf("\nDone.\n");
		return true;
	}
...
private:
	...
	bool checkCrc(const char* buf, unsigned len);
...
bool MSXhandle::checkCrc(const char* buf, unsigned len)
{
	...
	unsigned int w = getWord((unsigned char*)(buf + len));
	unsigned crcRecvd = checksum.value[1] + (checksum.value[0] << 8);
	return crcRecvd == w;
}
